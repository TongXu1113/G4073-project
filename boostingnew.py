...
def update_weights(df, side, mm):
    ...
    n = len(df.index)
    df['index1'] = range(n)
    n1 = abs(sum(df.query('y_' + sid + ' != 0')['y_'+sid]))
    if n1 * (mm + 1) >= n:
        n1 = n - n1 - 1
    else:
        n1 = n1 * mm
    ...
    learners = [i for i in df.columns if ((('action' in i) and ('buy' in i)) or (('action' in i) and ('sell' in i)))]
    M = len(learners)
    sortedLearners = []
    alphas = []
    ...
    for m in range(M):
        ...
        sa = np.random.choice(df.query('y_' + sid + ' == 0').index, size = n1, replace = False)
        df_sample = pd.concat([df.query('y_' + sid + ' != 0'), df.loc[sa]])
        df_sample.sort_index(axis = 0, inplace = True)
        sample_index = df_sample['index1']
        sw = [weights[i] for i in sample_index]

        for l in learners:
            errors.append(np.dot(sw, np.where(df_sample['y_' + sid] != np.abs(df_sample[l])*sidx, 1, 0))/sum(sw))
        ...
        errVect = np.where(df_sample['y_' + sid] != df_sample[minLearner], 1, 0)
